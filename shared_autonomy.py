...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scenario", type=str, help="circularroad, lanechange", default="lanechange"
    )
    ...
    ), "--scenario cannot be intersection for shared_autonomy.py"  # we don't have the optimal policy for that
    ...
    for _ in range(10):
        ...
        scores = np.array(
            [[1.0 / len(goals[scenario_name])] * len(goals[scenario_name])] * 100
        )
        while not done:
            t = time.time()
            ...
            for goal_id in range(len(goals[scenario_name])):
                if args.scenario.lower() == "circularroad":
                    optimal_action[
                        goals[scenario_name][goal_id]
                    ] = optimal_act_circularroad(env, goal_id)
                elif args.scenario.lower() == "lanechange":
                    optimal_action[
                        goals[scenario_name][goal_id]
                    ] = optimal_act_lanechange(env, goal_id)
            ...
            for goal_id in range(len(goals[scenario_name])):
                probs = mdns[goals[scenario_name][goal_id]](torch.Tensor(obs)).detach()
                probs = probs.numpy().reshape(-1)
                scores[:, goal_id] = scores[:, goal_id] * probs[goal_id]
            probs = np.sum(scores, axis=0)
            probs = probs / np.sum(probs)
            a_robot = np.array([0.0, 0.0])
            for goal_id in range(len(goals[scenario_name])):
                a_robot += probs[goal_id] * (np.reshape(optimal_action[goals[scenario_name][goal_id]],(-1)))
            a_robot[0] = np.clip(a_robot[0], -max_steering, max_steering)
            a_robot[1] = np.clip(a_robot[1], -max_throttle, max_throttle)
            ...
            a_human = np.array(
                [
                    interactive_policy.steering,
                    optimal_action[goals[scenario_name][0]][0, 1],
                ]
            ).reshape(1, -1)
